...
static int conv_uid (const char * name, union el * e)
{
    struct passwd *pwd;
...
static int conv_gid (const char * name, union el * e)
{
    struct group *grp;
...
static int conv_pgrp (const char * name, union el * e)
{
    if (! strict_atol (name, &e->num)) {
...
static int conv_sid (const char * name, union el * e)
{
    if (! strict_atol (name, &e->num)) {
...
static int conv_str (const char * name, union el * e)
{
    e->str = strdup (name);
    return 1;
}
...
static void output_numlist (const union el * list, int num)
{
    int i;
...
static void output_strlist (const union el * list, int num)
{
// FIXME: escape codes
    int i;
    const char *delim = opt_delim;
...
static union el * select_procs (int *num)
{
    ...
    while(readproc(ptp, &task)) {
        ...
        if (match ^ opt_negate) {	/* Exclusive OR is neat */
            ...
            if (matches == size) {
                size = size * 5 / 4 + 4;
                list = (el*) realloc(list, size * sizeof *list);
                if (list == NULL)
                    exit (EXIT_FATAL);
            }
            if (opt_long) {
...
int pgrep_P (int argc, char *argv[])
{
    union el *procs;
    int num;
