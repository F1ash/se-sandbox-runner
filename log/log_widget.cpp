    ...
    QMainWindow(parent)
{
    ...
    // see for LogWidget::saveLogToFile()
    lastProbe = true;
    setSizePolicy(
                QSizePolicy(
...
/* public slots */
void LogWidget::appendMsgToLog(QString &msg)
{
    Log->append(msg);
    Log->setToolTip(
...
/* private slots */
void LogWidget::timerEvent(QTimerEvent *ev)
{
...
void LogWidget::openLink(QUrl url)
{
    //qDebug()<<url.toString();
    QDesktopServices::openUrl(url);
}
void LogWidget::_saveLogToFile()
{
    lastProbe = true;
    saveLogToFile();
}
void LogWidget::saveLogToFile()
{
    if ( !lastProbe ) return;
...
QString LogWidget::getTemplateFilename() const
{
    return QString("SESandboxRunner_%1_%2.log")
            .arg(QDate::currentDate().toString("dd.MM.yyyy"))
            .arg(QTime::currentTime().toString());
}
void LogWidget::changeLogSize(int i)
{
    currLogSize = i;
    Log->setToolTip(QString("Event/Error Log\nMaxSize:\t%1 Bytes\nCurrent:\t%2")
                    .arg(currLogSize * ONE_MB)
